...
def capture(name,count=None):
    ...
    while success and cv2.waitKey(1)== -1 and index < count:
        ...
        success, frame = cameraCapture.read()
        index += 1
        print('capture process is ',index,' of ',count)
    cv2.destroyWindow("CameraWindow")
    cameraCapture.release()

def copy(name,count=2000):
            ...
            for filename in filenames:
                if filename.endswith('.jpg'):
                    ...
                    for i, d in enumerate(dets):
                        ...
                        # 保存图片
                        cv2.imwrite(output_dir + '/' + name + "/" + str("%04d" % k) + '.jpg', face)
                        k+=1
                        if k > count:
                            break;
                    # 不断刷新图像，频率时间为30ms
                    ...
                    if k > count:
                        break;
                print('face_decector process is ', k, ' of ', count)
        ...
        for filename in filenames:
            if filename.endswith('.jpg'):
                ...
                for i, d in enumerate(dets):
                    ...
                    #cv2.imshow('image',face)
                    # 保存图片
                    cv2.imwrite(output_dir+'/'+name+"/"+str("%04d" % index)+'.jpg', face)
                    index += 1
                    if index>count :
                        break;
                #不断刷新图像，频率时间为30ms
